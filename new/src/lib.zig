...
pub const DB = struct {
	...
	fn flush_memtable(self: DB) !void {
		const memtable = self.memtable;
		// Key Len + Key + Offset
		const index_size = memtable.keys_size() + (memtable.count() * 8);
		const index = try self.allocator.alloc(u8, index_size);
		defer self.allocator.free(index);

		// Key Len + Key + Value Len + Value Offset
		const data_size = (memtable.keys_size() + memtable.values_size()) + (memtable.count() * 8);
		const data = try self.allocator.alloc(u8, data_size);
		defer self.allocator.free(data);

		var iter = memtable.iterate();
		var index_i: usize = 0;
		var data_i: usize = 0;
...
pub const MemTable = struct {
    ...
    pub fn keys_size(self: MemTable) usize {
    	return self.entries.keys_size;
    }

    pub fn values_size(self: MemTable) usize {
    	return self.entries.values_size;
    }

    pub fn iterate(self: MemTable) RBTree.Iterator {
